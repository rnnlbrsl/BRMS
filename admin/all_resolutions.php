      ...
      if (!isset($_SESSION["usertype"])) 
{
    if ($_SESSION["usertype"] != 1) {
      ...
      $page = 'resolutions';
      $menu = 'all_resolutions';

if (isset($_POST['delete-id']))
{
    $id = $_POST['delete-id'];
    if ($stmt = $mysqli->prepare("DELETE FROM resolutions WHERE resolution_no = ? LIMIT 1"))
    {
...
elseif(isset($_POST['Update']))
{
    ...
    $stmt = $mysqli->prepare("UPDATE  resolutions SET official_id='$official_id', resolution_details='$details' WHERE resolution_no = ? LIMIT 1");
    $stmt->bind_param("i",$id);
    $stmt->execute();
    $stmt->close();
...
<html lang="en">
    ...
    <body>
		...
		<div class="panel panel-default sammacmedia">
            <div class="panel-heading">Bued Info. System All Resolutions</div>
                     ...
                     while($row=mysqli_fetch_array($query))
                        {
                          ...
                          <tr>
                            ...
                            <td><?php echo $row['resolution_details'];?></td>  
                            <td><?php echo $row['date_created'];?></td>  
                              
                              </td>
                            <td>
                              <a href="#resolution<?php echo $row['resolution_no']; ?>" data-toggle="modal">
                                <span class="fa fa-search-plus"></span></a>
                              <a href="#resolutionupdate<?php echo $row['resolution_no']; ?>" data-toggle="modal">
                                <span class="fa fa-pencil"></span></a>
                              <a href="javascript::void();" id="<?php echo $row['resolution_no']; ?>" class="delete"><span class="fa fa-trash-o"></span></a>
                              </td>
                              <form id="delete-form<?php echo $row['resolution_no']; ?>" method="post">
                                  <input type="hidden" value="<?php echo $row['resolution_no']; ?>" name="delete-id">
                              </form>
                          </tr>
                          <?php
                          require('resolution_info.php');
                          require('resolution_update.php');
                          $a++;
                      }
                      ?>
